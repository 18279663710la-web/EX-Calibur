...
`其他/` 是本地参考资料目录，已经加入 `.gitignore`，不会上传到 GitHub。
...
- `sync_ledger*.json`、`dify_backups/`、`backend/uploads/`、`frontend/node_modules/`、`其他/` 等本地运行产物和参考资料没有加入 Git。
- 公开部署前已替换默认数据库密码和默认种子用户密码。
...
- 后端定位为网关层：RAG 检索、知识库处理和复杂业务流程应放在 Dify 工作流与 Dify 知识库中完成。
